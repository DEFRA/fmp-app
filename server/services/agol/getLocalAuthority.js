...
const getLocalAuthority = (options) => {
  return esriFeatureRequestByIntersectArea(config.agol.localAuthorityEndPoint, options.geometry, options.geometryType)
    .then((esriResult) => {
      if (!esriResult || !Array.isArray(esriResult)) {
        throw new Error('Invalid response from AGOL localAuthority request')
      }
      const attributes = esriResult[0]?.attributes || {}
