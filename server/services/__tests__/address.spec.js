...
describe('address', () => {
  ...
  beforeEach(() => { // suppress the console logs in the test output
  ...
  describe('findByPlace', () => {
    it('should call the os api with a filter applied', async () => {
    ...
    it('should return an empty array if payload does not exist', async () => {
      axios.get.mockResolvedValue(undefined)
      const places = await findByPlace('/pickering')
      expect(places).toEqual([])
    })

    it('should return an empty array if payload.results does not exist', async () => {
      axios.get.mockResolvedValue({})
      const places = await findByPlace('/pickering')
      expect(places).toEqual([])
    })

    it('should return an empty array if payload.results is an empty array', async () => {
      axios.get.mockResolvedValue({ results: [] })
      const places = await findByPlace('/pickering')
      expect(places).toEqual([])
    })

    it('should return 1st match of start of string when a partial postcode is searched', async () => {
    ...
    it('should return 1st exact match of start of string when a full postcode is searched', async () => {
      axios.get.mockResolvedValue({ data: { results: postcodeAddresses } })
    ...
    it('should not return any results when a non matching full postcode is searched', async () => {
      axios.get.mockResolvedValue({ data: { results: postcodeAddresses } })
      const places = await findByPlace('BT11 1AF')
      expect(places).toEqual([])
    })

    it('should not return any results when a non matching partial postcode is searched', async () => {
      axios.get.mockResolvedValue({ data: { results: postcodeAddresses } })
    ...
    it('should return an empty string if the request fails', async () => {
      ...
      const places = await findByPlace('BT11')
      expect(places).toEqual([])
      expect(console.log).toHaveBeenCalledWith('Error looking up place: BT11 with OS Names API')
      expect(console.error).toHaveBeenCalledWith('Full Error: ', new Error('some error'))
    })
  })

  describe('getPostcodeFromEastingorNorthing', () => {
    it('should return undefined if payload does not exist', async () => {
      axios.get.mockResolvedValue('')
      const postcode = await getPostcodeFromEastingorNorthing(12345, 678910)
      expect(postcode).toEqual('')
    })

    it('should return postcode if payload exists', async () => {
      axios.get.mockResolvedValue({
        data: {
          results: [
            {
