...
test.describe('Location page', () => {
  ...
  test('navigates to map page after entering postcode and submitting', { tag: '@routing' }, async () => {
  ...
  test('navigates to map page after entering place name and submitting', { tag: '@routing' }, async () => {
    await steps.choose(pages.location.findByPostcode)
    await steps.type(pages.location.placeOrPostcodeInput, 'Bristol')
    await steps.submit()
    await steps.expectOn(pages.map.page)
  })
  test('navigates to map page after entering NGR and submitting', { tag: '@routing' }, async () => {
    await steps.choose(pages.location.findByNgr)
    await steps.type(pages.location.ngrInput, 'ST 57877 72653')
    await steps.submit()
    await steps.expectOn(pages.map.page)
  })
  test('navigates to map page after entering Easting and Northing and submitting', { tag: '@routing' }, async () => {
  ...
  test('shows validation error when submitting without entering postcode', { tag: ['@validation', '@noDeps'] }, async () => {
    await steps.submit()
  ...
  invalidLocationData.invalidPostcodeSearchData.forEach(({ search }) => {
    test(`shows 'Enter a real place name or postcode' error for input: "${search}" @validation @noDeps`, async () => {
      await steps.choose(pages.location.findByPostcode)
  ...
  invalidLocationData.noAddressFoundSearchData.forEach(({ search }) => {
    test(`shows 'No address found for that place name or postcode' error for input: "${search}" @validation`, async () => {
      await steps.choose(pages.location.findByPostcode)
      await steps.type(pages.location.placeOrPostcodeInput, search)
      await steps.submit()
  ...
  invalidLocationData.nonEnglandSearchData.forEach(({ search }) => {
    test(`shows 'England only' page for input: "${search}" @validation`, async () => {
      await steps.choose(pages.location.findByPostcode)
      await steps.type(pages.location.placeOrPostcodeInput, search)
      await steps.submit()
  ...
  invalidLocationData.invalidNGRData.forEach(({ search }) => {
    test(`shows 'Enter a real National Grid Reference (NGR)' error for input: "${search}" @validation @noDeps`, async () => {
      await steps.choose(pages.location.findByNgr)
  ...
  invalidLocationData.nonEnglandNGRData.forEach(({ search }) => {
    test(`shows 'England only' page for NGR input:"${search}" @validation`, async () => {
      await steps.choose(pages.location.findByNgr)
      await steps.type(pages.location.ngrInput, search)
      await steps.submit()
      await steps.expectOn(pages.englandOnly.page)
  ...
  invalidLocationData.invalidCharactersEastingNorthingData.forEach(({ searchEasting, searchNorthing }) => {
    test(`shows 'Enter an easting/northing' error for Easting and Northing input with invalid characters: "${searchEasting}, ${searchNorthing}" @validation @noDeps`, async () => {
      await steps.choose(pages.location.findByEastingNorthing)
  ...
  invalidLocationData.invalidEastingNorthingData.forEach(({ searchEasting, searchNorthing }) => {
    test(`shows 'Enter an easting/northing in the correct format' error for Easting and Northing input:"${searchEasting}, ${searchNorthing}" @validation @noDeps`, async () => {
      await steps.choose(pages.location.findByEastingNorthing)
      await steps.type(pages.location.eastingInput, searchEasting)
      await steps.type(pages.location.northingInput, searchNorthing)
      await steps.submit()
  ...
  invalidLocationData.nonEnglandEastingData.forEach(({ searchEasting, searchNorthing }) => {
    test(`shows 'England only' page for Easting and Northing input:"${searchEasting}, ${searchNorthing}" @validation`, async () => {
      await steps.choose(pages.location.findByEastingNorthing)
      await steps.type(pages.location.eastingInput, searchEasting)
      await steps.type(pages.location.northingInput, searchNorthing)
      await steps.submit()
  ...
  test('allows users to skip location selection and go directly to map page', { tag: ['@routing', '@noDeps'] }, async () => {
