...
test.describe('Map help page', { tag: '@noDeps' }, () => {
  ...
  test('displays map help content', { tag: '@validation' }, async () => {
    await steps.expectText('Help using the flood map')
  })
  test('confirms the accessibility link is present@routing', async () => {
    await steps.expectLinkExists(pages.mapHelp.accessibilityLink)
  })
  test('confirms the get more information about a specific location link is present@routing', async () => {
    await steps.expectLinkExists(pages.mapHelp.getMoreInformationLink)
  })
  test('confirms the draw a boundary link is present@routing', async () => {
    await steps.expectLinkExists(pages.mapHelp.drawABoundaryLink)
  })
  test('confirms the how to use the data link is present@routing', async () => {
    await steps.expectLinkExists(pages.mapHelp.howToUseDataLink)
  })
  test('confirms the other ways to get flood risk information link is present@routing', async () => {
    await steps.expectLinkExists(pages.mapHelp.otherWaysToGetFloodRiskInformationLink)
  })

  test('navigates to the how to use flood map for planning data page when clicking the link', { tag: '@routing' }, async () => {
    await steps.clickLink(pages.mapHelp.findOutMoreLink)
    await steps.expectOn(pages.howToUseFloodMapForPlanningData.page)
  })
  test('navigates to the accessibility statement page when clicking the link', { tag: '@routing' }, async () => {
