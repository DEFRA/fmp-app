...
test.describe('Privacy notice page', { tag: '@noDeps' }, () => {
  ...
  test('navigates to the General Data Protection Regulation page when clicking the link', { tag: '@urlCheck' }, async ({ steps }) => {
    await steps.clickLink(pages.privacyNotice.gdprLink)
    await steps.expectUrlContains('eur/2016/679/')
  })

  test('navigates to the Data Protection Act page when clicking the link', { tag: '@urlCheck' }, async ({ steps }) => {
    await steps.clickLink(pages.privacyNotice.dataProtectionActLink)
    await steps.expectUrlContains('data-protection')
  })

  test('navigates to the personal information charter page when clicking the link', { tag: '@urlCheck' }, async ({ steps }) => {
  ...
  test('navigates to the web browser page when clicking the link', { tag: '@urlCheck' }, async ({ steps }) => {
  ...
  test('navigates to the access your data page when clicking the link', { tag: '@urlCheck' }, async ({ steps }) => {
    await steps.clickLink(pages.privacyNotice.accessYourDataLink)
    await steps.expectUrlContains('data-protection')
  })

  test('navigates to the Information Commissioner\u2019s Office page when clicking the link', { tag: '@urlCheck' }, async ({ steps }) => {
    await steps.clickLink(pages.privacyNotice.informationCommissionersOfficeLink)
    await steps.expectUrlContains('ico.org.uk')
  })

  test('navigates to the contact us page when clicking the link', { tag: '@urlCheck' }, async ({ steps }) => {
    await steps.clickLink(pages.privacyNotice.contactUsLink)
    await steps.expectUrlContains('contact-us')
  })

  test('navigates to the GOV.UK Verify page when clicking the link', { tag: '@urlCheck' }, async ({ steps }) => {
